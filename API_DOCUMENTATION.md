...
`reviewIntervals` contains the preview interval in days for each supported rating, calculated using the backend SM-2 scheduling logic:
...
These values are previews only. They do not update the flashcard's stored scheduling state. The actual schedule changes only when the review endpoint is called.
