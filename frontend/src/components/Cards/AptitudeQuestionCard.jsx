...
const AptitudeQuestionCard = ({ question, options, answer }) => {
  ...
  return (
    <div className="relative bg-white dark:bg-white/10 backdrop-blur-md border border-border-color dark:border-white/20 rounded-xl p-5 mb-4 shadow-sm hover:shadow-md transition-shadow duration-300">
      {/* Expand button */}
      <button
        className="absolute top-4 right-4 text-gray-400  hover:text-violet-600 dark:hover:text-violet-400 transition-colors"
      ...
      >
        ...
        <h3 className="text-text-primary/90 font-medium text-base leading-snug pt-0.5">
          {question}
        </h3>
      </div>
      ...
      {options && Array.isArray(options) && (
        <div className="mt-5 space-y-2">
          {options.map((opt, i) => {
            ...
            let optionStyles ="bg-surface border border-border-color text-gray-700  hover:bg-violet-50 dark:hover:bg-white/5 cursor-pointer";
            ...
            if (selectedOption !== null) {
                ...
                if (isCorrect) {
                  ...
                  optionStyles ="bg-surface opacity-60 border-border-color text-text-muted cursor-not-allowed";
                  labelStyles ="text-text-muted";
                }
            }
