    ...
    <div className="min-h-screen bg-background text-text-primary flex flex-col transition-colors duration-300">
      <main className="flex-1 container mx-auto px-4 py-10 md:py-16">
        ...
        <div className={`text-center mb-12 transition-colors duration-300 ${selectedGame ?"hidden" :""}`}>
          ...
          <p className="text-text-muted font-medium max-w-2xl mx-auto md:text-lg">
            ...
            {gamesData.map((game) => {
              ...
              return (
                <button
                  ...
                  className="group flex flex-col items-start sm:flex-row sm:items-center gap-4 md:gap-5 p-5 md:p-6 rounded-2xl bg-card border border-border-color hover:border-violet-300 dark:hover:border-violet-500/40 hover:bg-violet-50/50 dark:hover:bg-violet-900/10 transition-all duration-300 text-left w-full shadow-sm hover:shadow"
                >
                  <div className="flex-shrink-0 w-12 h-12 md:w-14 md:h-14 flex items-center justify-center rounded-xl bg-gray-50 dark:bg-black/20 border border-gray-100  text-text-secondary group-hover:bg-violet-100 group-hover:text-violet-600 dark:group-hover:bg-violet-600/30 dark:group-hover:text-violet-400 transition-colors duration-300">
                    <Icon className="w-5 h-5 md:w-6 md:h-6" />
                  </div>
                  <div className="flex flex-col mt-2 sm:mt-0 flex-1">
                    <h3 className="text-[17px] font-bold text-text-primary mb-1 group-hover:text-violet-600 dark:group-hover:text-violet-400 transition-colors duration-300">
                      {game.name}
                    </h3>
                    <p className="text-[13px] md:text-sm text-text-muted leading-snug line-clamp-2">
                      {game.desc}
                    </p>
                  </div>
                </button>
        ...
        {/* Active game header */}
        {selectedGame && (
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-8 pb-4 border-b border-border-color transition-colors duration-300">
            ...
            <button
              className="bg-gray-100 hover:bg-gray-200 dark:bg-white/10 dark:hover:bg-white/20 text-text-secondary px-4 py-2 rounded-lg font-medium shadow-sm transition-colors duration-300 flex items-center gap-2"
              onClick={handleBack}
            >
              ← Choose Another Game
            </button>
