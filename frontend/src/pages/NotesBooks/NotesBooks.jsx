  ...
  return (
    <div className="min-h-full bg-[var(--color-background)] text-[var(--color-text-dark)]">
      <div className="max-w-6xl mx-auto p-6 md:p-8 space-y-8">
        <div className="flex flex-col gap-4 bg-card border border-border-color rounded-2xl p-6 shadow-sm">
            ...
            <div className="flex items-center gap-2">
              <div className="relative w-full md:w-72">
                ...
                <input
                  ...
                  className="w-full pl-9 pr-3 py-2 rounded-lg border border-border-color bg-transparent text-sm focus:outline-none focus:ring-2 focus:ring-violet-400/70"
                />
              </div>
              <button
                onClick={fetchBooks}
                className="inline-flex items-center gap-2 px-3 py-2 text-sm font-medium rounded-lg border border-border-color bg-card hover:border-violet-300 dark:hover:border-violet-400/50 transition-colors"
              >
                <RefreshCw size={16} />
                Refresh
              </button>
        ...
        {loading ? (
          <div className="grid sm:grid-cols-2 gap-4">
            {[1, 2, 3, 4].map((n) => (
              <div
                key={n}
                className="animate-pulse bg-card border border-border-color rounded-xl p-5 h-36"
              />
            ))}
          </div>
        ) : filteredCategories.length === 0 ? (
          <div className="text-center py-16 bg-card border border-border-color rounded-2xl">
          ...
          <div className="grid md:grid-cols-2 gap-4">
            {filteredCategories.map((category) => (
              <div
                key={category.id}
                className="bg-card border border-border-color rounded-2xl p-5 space-y-4"
              >
                ...
                <div className="space-y-2">
                  {(category.items || []).map((item) => (
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="flex items-center gap-3 p-3 rounded-lg border border-border-color hover:border-violet-300 dark:hover:border-violet-400/50 hover:bg-violet-50/40 dark:hover:bg-violet-500/5 transition-colors"
                    >
                      <div className="w-9 h-9 rounded-lg bg-surface flex items-center justify-center text-text-secondary">
                        <FileText size={16} />
                      </div>
