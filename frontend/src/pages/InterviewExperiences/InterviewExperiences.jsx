...
const EXPERIENCES = [
  {
    ...
    tips: ["Practice 150+ LeetCode problems; focus on Trees, Graphs and DP.","For system design, always start with clarifying requirements and estimate scale.","Googliness questions are NOT casual — prepare strong STAR-format stories.","Use 'Think aloud' strategy throughout coding rounds.",
    ],
  ...
  },
  {
    ...
    tips: ["Memorise and map all 16 Leadership Principles to real work stories.","Every round has LP questions — have 4-5 strong STAR stories prepared.","For system design focus on scalability, availability and cost trade-offs.","Always clarify edge cases before coding.",
    ],
  ...
  },
  {
    ...
    tips: ["Microsoft values clean code and good OOP design — always structure your classes.","Practice design patterns: Singleton, Factory, Observer.","Interviewers are collaborative; feel free to ask for clarifications.","Research the team and product line before your interview.",
    ],
  ...
  },
  {
    ...
    tips: ["Brush up on React internals — reconciliation, fiber, hooks in depth.","Practice building mini UI components from scratch under time pressure.","Meta values 'Move fast' — show quick decision-making while coding.","System design: emphasise real-time messaging patterns.",
    ],
  ...
  },
  {
    ...
    tips: ["Practice greedy and DP problems — very common at Flipkart.","Know your resume projects deeply; they often form system design topics.","For HR round negotiate confidently with data points.",
    ],
  ...
  },
  {
    ...
    tips: ["Write well-documented, tested code for the take-home — quality matters more than speed.","Practice defending your technical choices calmly under pressure.","Atlassian's values (Open, Honest, Team-first) are central to the culture interview.",
    ],
  ...
  },
  {
    ...
    tips: ["Know idempotency and distributed transaction patterns well.","Understand payment flows end-to-end — PSP, bank APIs, settlements.","Show ownership and startup hustle in behavioral rounds.",
    ],
  ...
  },
  {
    ...
    tips: ["Practice core DSA — arrays, strings, linked lists and trees.","Show enthusiasm for food-tech and hyper-local problems.","Know Big-O complexity for every solution you write.",
    ],
  ...
  },
  {
    ...
    tips: ["Have your internship project explained deeply — interviewers will probe it.","Practice basic graph and tree traversals.","Be clear about your expectations and enthusiasm for fintech.",
    ],
  ...
  },
  {
    ...
    tips: ["Always quantify impact in behavioral stories (e.g., reduced latency by 30%).","Know real-time architecture patterns like WebSockets and event streaming.","Zomato values hustle and agility — show quick execution examples.",
    ],
...
const ROUND_TYPE_ICON = {
  Coding:      Code2,
  DSA:         Brain,"System Design": Layers,
  Behavioral:  Users,
  Backend:     Briefcase,
  General:     MessageSquare,
  Final:       Trophy,"UI Design": Code2,"Code Review": Code2,"API Design": Code2,"Problem Solving": Brain,
};
...
const ExperienceCard = ({ exp, onClick }) => (
  <div
    onClick={() => onClick(exp)}
    className="group relative bg-white dark:bg-[#151c2f] border border-border-color  rounded-2xl p-5 cursor-pointer hover:border-violet-500/30 hover:shadow-lg hover:shadow-violet-500/5 hover:-translate-y-0.5 transition-all duration-300 flex flex-col gap-4"
  >
    ...
    <div className="flex items-start gap-3">
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 flex-wrap">
          <h3 className="text-sm font-bold text-text-primary truncate">{exp.company}</h3>
          <DifficultyBadge difficulty={exp.difficulty} />
        </div>
        <p className="text-xs text-text-muted mt-0.5 truncate">{exp.role}</p>
      </div>
    </div>

    {/* Stats row */}
    <div className="flex items-center gap-3 flex-wrap text-[11px] font-medium text-text-muted">
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      <div
        ...
        onClick={onClose}
      />
      <div className="relative z-10 w-full max-w-2xl max-h-[90vh] overflow-y-auto bg-white dark:bg-sidebar border border-border-color rounded-3xl shadow-2xl custom-scrollbar">
        {/* Header */}
        <div className="sticky top-0 z-20 bg-white dark:bg-sidebar border-b border-gray-100  px-6 py-5 flex items-start gap-4">
          <CompanyInitial company={exp.company} color={exp.color} />
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 flex-wrap">
              <h2 className="text-lg font-extrabold text-text-primary">{exp.company}</h2>
              <DifficultyBadge difficulty={exp.difficulty} />
            ...
            <p className="text-sm text-text-muted mt-0.5">{exp.role} · {exp.experience} exp · {exp.date}</p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="px-6 py-6 space-y-8">
          {/* Summary */}
          <div>
            <h4 className="text-[11px] font-extrabold uppercase tracking-widest text-text-muted dark:text-text-muted mb-3">Overview</h4>
            <p className="text-sm text-gray-700  leading-relaxed">{exp.summary}</p>
          </div>

          {/* Rounds */}
          <div>
            <h4 className="text-[11px] font-extrabold uppercase tracking-widest text-text-muted dark:text-text-muted mb-4">Interview Rounds</h4>
                  ...
                  <div key={i} className="flex gap-4">
                    ...
                    <div className="pb-4">
                      <div className="flex items-center gap-2 mb-1 flex-wrap">
                        <span className="text-sm font-bold text-text-primary">{round.name}</span>
                      ...
                      <p className="text-xs text-text-secondary  leading-relaxed">{round.description}</p>
                    </div>
                  </div>
                );
              })}
          ...
          {/* Tips */}
          <div>
            <h4 className="text-[11px] font-extrabold uppercase tracking-widest text-text-muted dark:text-text-muted mb-3">Tips & Advice</h4>
...
const SubmitModal = ({ onClose, onAdd }) => {
  ...
  const inputCls ="w-full bg-gray-50 dark:bg-black/20 border border-border-color rounded-xl px-4 py-3 text-sm text-text-primary placeholder-gray-400 dark:placeholder-gray-600 outline-none focus:ring-2 focus:ring-violet-500/40 focus:border-violet-500/60 transition-all";
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      <div
        ...
        onClick={onClose}
      />
      <div className="relative z-10 w-full max-w-xl max-h-[90vh] overflow-y-auto bg-white dark:bg-sidebar border border-border-color rounded-3xl shadow-2xl custom-scrollbar">
        {/* Header */}
        <div className="sticky top-0 z-20 bg-white dark:bg-sidebar border-b border-gray-100  px-6 py-5 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-xl bg-violet-500/10 border border-violet-500/20 flex items-center justify-center">
              <Plus size={18} className="text-violet-400" />
            </div>
            <h2 className="text-base font-extrabold text-text-primary">Share Your Experience</h2>
          </div>
        ...
        <div className="px-6 py-6">
          {submitted ? (
            <div className="flex flex-col items-center justify-center py-12 text-center gap-4">
              ...
              <h3 className="text-lg font-extrabold text-text-primary">Thank you!</h3>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-[11px] font-bold uppercase tracking-wider text-text-muted mb-1.5">Company *</label>
              ...
              <div className="grid grid-cols-3 gap-4">
                <div>
                  <label className="block text-[11px] font-bold uppercase tracking-wider text-text-muted mb-1.5">Experience</label>
                  <input name="experience" value={form.experience} onChange={handleChange} placeholder="e.g. 3 Years" className={inputCls} />
                </div>
                <div>
                  <label className="block text-[11px] font-bold uppercase tracking-wider text-text-muted mb-1.5">Difficulty</label>
                ...
                <div>
                  <label className="block text-[11px] font-bold uppercase tracking-wider text-text-muted mb-1.5">Offer?</label>
              ...
              <div>
                <label className="block text-[11px] font-bold uppercase tracking-wider text-text-muted mb-1.5">Rounds Summary *</label>
                <textarea
              ...
              <div>
                <label className="block text-[11px] font-bold uppercase tracking-wider text-text-muted mb-1.5">Overall Summary *</label>
                <textarea
              ...
              <div>
                <label className="block text-[11px] font-bold uppercase tracking-wider text-text-muted mb-1.5">Tips for Others</label>
                <textarea
...
const CompanyDropdown = ({ value, onChange, options }) => {
  ...
  return (
    <div ref={ref} className="relative shrink-0">
      ...
      <button
        ...
        className={`flex items-center gap-2 px-3 py-2 rounded-xl border text-sm font-medium transition-all ${
          open
            ...
            :"bg-gray-50 dark:bg-black/20 border-border-color text-gray-700  hover:border-violet-400/50 hover:text-violet-300"
        }`}
      >
      ...
      {/* Dropdown panel */}
      {open && (
        <div className="absolute right-0 top-[calc(100%+6px)] z-50 w-52 bg-[#1a2236] dark:bg-sidebar border border-white/10 rounded-2xl shadow-2xl shadow-black/40 overflow-hidden">
          {/* Header label */}
...
const InterviewExperiences = () => {
  ...
  return (
    <div className="min-h-screen bg-background px-5 py-10 md:px-10 transition-colors duration-300">
      <div className="max-w-6xl mx-auto space-y-8">
        ...
        <div className="grid grid-cols-3 gap-4">
          {[
            ...
            <div key={label} className="bg-white dark:bg-[#151c2f] border border-border-color  rounded-2xl p-4 md:p-5 flex items-center gap-4">
        ...
        {/* ── Filters ── */}
        <div className="bg-white dark:bg-[#151c2f] border border-border-color  rounded-2xl p-4 flex flex-col sm:flex-row gap-3">
          {/* Search */}
          <div className="flex-1 flex items-center gap-2 bg-gray-50 dark:bg-black/20 border border-border-color rounded-xl px-3 py-2.5">
            <Search size={15} className="text-gray-400 shrink-0" />
            <input
              value={search}
              onChange={(e) => setSearch(e.target.value)}
              placeholder="Search company, role, or topic..."
              className="flex-1 bg-transparent text-sm text-text-primary placeholder-gray-400 dark:placeholder-gray-600 outline-none"
            />
          </div>
                ...
                <button
                  ...
                  className={`text-[11px] font-bold px-3 py-1.5 rounded-full border transition-all ${
                    diffFilter === f
                      ...
                      :"bg-transparent border-border-color text-text-muted hover:border-violet-400 hover:text-violet-400"
                  }`}
                >
                  {f}
                </button>
        ...
        <div className="flex items-center justify-between gap-4">
          {/* Tab toggle pill */}
          <div className="flex items-center gap-1 p-1 bg-white dark:bg-[#151c2f] border border-border-color  rounded-xl">
            {["Common","User"].map((tab) => {
        ...
        {activeTab ==="User" && userExperiences.length === 0 ? (
          ...
          <div className="flex flex-col items-center justify-center py-24 text-center border border-dashed border-violet-500/20 rounded-2xl bg-violet-500/5 dark:bg-violet-900/5">
            ...
            <h3 className="text-base font-bold text-text-primary mb-2">No experiences shared yet</h3>
          ...
          <div className="flex flex-col items-center justify-center py-24 text-center border border-border-color  rounded-2xl bg-white dark:bg-[#151c2f]">
            <div className="w-14 h-14 rounded-2xl bg-violet-500/10 border border-violet-500/20 flex items-center justify-center mb-4">
              <Star size={24} className="text-violet-400" />
            </div>
            <h3 className="text-base font-bold text-text-primary mb-2">No experiences found</h3>
            <p className="text-sm text-text-muted max-w-xs">
