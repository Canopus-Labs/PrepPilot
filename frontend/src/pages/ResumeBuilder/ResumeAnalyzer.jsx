        ...
        {!result ? (
          ...
          <div className="animate-in fade-in slide-in-from-bottom-8 duration-700 ease-out space-y-8">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
              ...
              <div className="lg:col-span-3 flex flex-col sm:flex-row items-center justify-around bg-white dark:bg-[#111827] rounded-[2.5rem] p-8 md:p-12 border border-gray-200 dark:border-white/5 shadow-xl relative overflow-hidden">
                 {/* Decorative vectors */}
                 ...
                 {renderScoreRing(result.resumeScore, "Resume Score", "Overall quality")}
                 
                 <div className="hidden sm:block w-px h-24 bg-gradient-to-b from-transparent via-gray-200 dark:via-white/10 to-transparent"></div>
                 
                 {renderScoreRing(result.roleMatch, "Role Match", `Target: ${targetRole || "General"}`)}

                 <div className="hidden sm:block w-px h-24 bg-gradient-to-b from-transparent via-gray-200 dark:via-white/10 to-transparent"></div>

                 {renderScoreRing(result.atsScore || result.resumeScore, "ATS Score", "Parser friendliness")}
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
               ...
               <div className="space-y-6">
                   ...
                   {result.strengths?.length > 0 ? (
                     <ul className="space-y-3">
                       {result.strengths.map((strength, i) => (
                         <li key={i} className="flex items-start gap-3 text-sm font-medium text-gray-700 dark:text-gray-300 bg-emerald-50/50 dark:bg-emerald-900/10 p-3 rounded-xl border border-emerald-100 dark:border-emerald-500/10">
                     ...
                     <p className="text-sm text-gray-500 dark:text-gray-400 font-medium">No specific strengths highlighted.</p>
                   )}
                 </div>
                 ...
                 <div className="bg-white dark:bg-[#151c2f] rounded-3xl p-8 border border-gray-200 dark:border-white/5 shadow-sm">
                   <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-6 flex items-center gap-2">
                     <AlertTriangle size={20} className="text-red-500" /> Areas for Improvement
                   </h3>
                   {result.weaknesses?.length > 0 ? (
                     <ul className="space-y-3">
                       {result.weaknesses.map((weakness, i) => (
                         <li key={i} className="flex items-start gap-3 text-sm font-medium text-gray-700 dark:text-gray-300 bg-red-50/50 dark:bg-red-900/10 p-3 rounded-xl border border-red-100 dark:border-red-500/10">
                     ...
                     <p className="text-sm text-gray-500 dark:text-gray-400 font-medium">No major weaknesses detected.</p>
                   )}
                 </div>
               </div>
               ...
               <div className="space-y-6">
                 ...
                 <div className="bg-white dark:bg-[#151c2f] rounded-3xl p-8 border border-gray-200 dark:border-white/5 shadow-sm">
                   <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-6 flex items-center gap-2">
                     <Target size={20} className="text-amber-500" /> Critical Skills & Keyword Gaps
                   </h3>
                   
                   <div className="space-y-4">
                     {result.missingSkills?.length > 0 && (
                       <div>
                         <h4 className="text-xs font-bold uppercase tracking-wider text-gray-400 mb-3">Missing Skills</h4>
                     ...
                     {result.keywordGaps?.length > 0 && (
                       <div>
                         <h4 className="text-xs font-bold uppercase tracking-wider text-gray-400 mb-3">Keyword Gaps</h4>
                         <div className="flex flex-wrap gap-2">
                           {result.keywordGaps.map((keyword, i) => (
                             <span key={i} className="px-3 py-1.5 rounded-lg bg-amber-50 dark:bg-amber-900/20 text-amber-600 dark:text-amber-400 text-sm font-bold border border-amber-100 dark:border-amber-500/20">
                               {keyword}
                             </span>
                           ))}
                         </div>
                     ...
                     {!result.missingSkills?.length && !result.keywordGaps?.length && (
                       <p className="text-sm text-gray-500 dark:text-gray-400 font-medium">No critical missing skills or keyword gaps detected!</p>
                     )}
                   </div>
                 </div>
                 ...
                 <div className="bg-white dark:bg-[#151c2f] rounded-3xl p-8 border border-gray-200 dark:border-white/5 shadow-sm">
                   ...
                   <ul className="space-y-3">
                     {result.missingProjects?.length > 0 ? result.missingProjects.map((proj, i) => (
                       ...
                       <p className="text-sm text-gray-500 dark:text-gray-400 font-medium">Your portfolio matches role expectations.</p>
                     )}
                   </ul>
                 </div>
               </div>
               ...
               {/* Suggestions List */}
               <div className="md:col-span-2 bg-white dark:bg-[#151c2f] rounded-3xl p-8 border border-gray-200 dark:border-white/5 shadow-sm flex flex-col">
                 <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-6 flex items-center gap-2">
                   <Zap size={20} className="text-emerald-500" /> Detailed Improvement Suggestions
                 </h3>
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                   {result.suggestions?.map((sug, i) => (
                      <div key={i} className="flex items-start gap-3 bg-gray-50 dark:bg-white/5 p-4 rounded-2xl border border-gray-100 dark:border-white/5">
