...
router.post('/compile', protect, checkRole(['user', 'moderator', 'admin']), aiLimiter, compileResume);

// @route   POST /api/resume/analyze
// @desc    Analyze resume using Gemini API
// @access  Private — requires auth; aiLimiter caps Gemini API calls to 20/hr per IP
router.post('/analyze', protect, checkRole(['user', 'moderator', 'admin']), aiLimiter, uploadResume.single("resume"), analyzeResume);

// @route   POST /api/resume/save
// @desc    Save or update a resume
// @access  Private
router.post('/save', protect, checkRole(['user', 'moderator', 'admin']), saveResume);

// @route   GET /api/resume/my-resumes
// @desc    Get all saved resumes for logged-in user
// @access  Private
router.get('/my-resumes', protect, checkRole(['user', 'moderator', 'admin']), getMyResumes);
